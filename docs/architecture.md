...
Sprint 3.9.1 adds a Core-owned signal-health state machine above existing RMS events. Each selected audio stream starts in a 30-second grace period, becomes healthy only after finite RMS evidence above -55 dB, reports `NeverDetected` when that evidence never arrives and `CurrentlySilent` after 45 seconds without renewed evidence. The desktop evaluates state on its existing recording timer, renders a non-blocking amber source-specific warning, writes only transitions to the bounded status log and clears automatically on recovery. Health observations do not modify PCM, stop capture or claim conferencing-app mute semantics.

Sprint 3.9.2 performs a deterministic PCM16 signal preflight after normalization and before importing Torch, Whisper or Pyannote. Per-source evidence records duration, peak/RMS, strongest 100 ms window and active duration using the same -55 dBFS threshold as live health. Sources with less than 200 ms usable evidence produce a valid skipped-source result; another useful track continues independently, while an entirely silent job completes with zero segments without loading an ML model. The evidence and skipped-source list remain in the versioned transcript JSON.

Sprint 3.9.3 activates `IAudioMeterInformation` and `IAudioEndpointVolume` directly on each discovered Windows audio endpoint. Probes run off the UI thread, at most once per second and only while the selected stream is already in a warning state. A selected global endpoint mute is reported explicitly; otherwise an unmuted same-kind endpoint above -50 dBFS may be suggested as an active alternative. The advisor never changes selection or mute state. Windows microphone-use/privacy indicators and endpoint mute do not expose a reliable conferencing-app in-call mute, so Teams mute remains informationally unknown and must not control capture.

The visible live meters intentionally share the -55 dBFS useful-signal floor with capture health; lower driver/Teams comfort noise is recorded but no longer presented as meaningful activity. A meter also decays to zero after 750 ms without a new WASAPI packet so a loopback endpoint cannot leave its last spoken level frozen on screen.

Sprint 3.9.4 evaluates Teams-specific mute state without Graph credentials or input interception. Microsoft Graph exposes `participant.isMuted` only in a Cloud Communications call context requiring an application/bot to join and receive roster updates; it is not a lightweight local-client state API. The desktop therefore uses read-only Windows UI Automation against the current Teams `microphone-button`. Its localized accessible action name is mapped to `Muted`, `Unmuted` or `Unknown`, cached after discovery and polled once per second only during capture. This slice is diagnostic: it displays and logs state transitions but intentionally continues recording the local microphone until hardware validation proves click, shortcut, device-switch and meeting-lifecycle reliability.

Sprint 3.9.5 promotes the hardware-validated Teams state to capture policy. While Teams is positively identified as muted, the microphone WASAPI provider replaces PCM samples with zeroes before WAV output, level reporting and live-chunk publication. The endpoint remains open and frame counts continue, preserving synchronization across microphone, system audio and video. `Unknown` and `NotDetected` fail open and restore normal local capture; the integration never clicks or changes Teams itself.

Sprint 3.9.6 adds persistent, independent 25–150% digital recording levels for system audio and microphone. Scaling occurs on PCM16 immediately after format conversion and before WAV output, metering and live chunks, with saturating clipping protection instead of integer wraparound. A 100% setting is bit-neutral. These controls can attenuate an overly hot but unclipped endpoint; they cannot reconstruct samples already clipped by microphone hardware or its Windows driver.

Sprint 4.0 replaces the organically grown dashboard with a stable application shell. The left two-thirds contain productive capture and transcription work; the right third is a persistent assistant surface for avatar reactions, operational guidance, warnings and collapsible technical activity. Header navigation keeps recordings, the latest transcript and settings directly reachable. Storage, future minutes and other domains can become dedicated workspaces without adding more unrelated cards to the capture screen. This foundation intentionally reuses every existing command and dialog before introducing new behavior.

The v0.4.0 capture page exposes a persistent Live transcription choice. When disabled before recording, the combined coordinator does not create incremental audio writers, the desktop ignores chunk events and automatic finalization is not queued. Finalized WAV/MP4 masters and the session manifest remain unchanged, so the meeting can be transcribed manually later. The choice cannot change mid-capture and survives restart.
