...
- **Sprint 2.7 / AMA-117: Meeting Library** — Replace the single `latest` affordance with a durable local session browser. Enumerate every capture workspace and show date/time, duration, capture/recovery status, source availability, alignment warning and transcript state. Support opening an existing transcript and explicitly starting/retrying/cancelling transcription for any eligible completed session. Later add rename/title, search, export, retention and guarded deletion. Corrupt/incomplete sessions remain visible with diagnostics rather than disappearing.
