...
6. **AMA-106: Session workspace** — **Sprint 1.6.1 complete.** Each recording creates a unique `session_*` directory containing all three media artifacts and an atomically replaced, versioned `manifest.json`. A shared monotonic clock records the actual provider start signals as per-stream millisecond offsets plus total session duration and selected source IDs. Real validation measured start offsets of 1047.6 ms (screen), 1091.6 ms (system audio) and 1248.7 ms (microphone); offset plus media duration placed all three stream ends within 31 ms. Resume/recovery and free-disk validation remain later slices.
