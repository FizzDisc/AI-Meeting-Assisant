...
- **Sprint 2.3 complete:** supervise real WhisperX transcription in a disposable child process with `start/status/cancel` protocol calls. Normalize one or mix two WAV inputs via FFmpeg, require an explicitly installed local model directory and atomically persist schema-versioned timestamped segments. Cancellation kills native inference without taking down the worker. Model downloads use the native Windows certificate store through pinned `truststore` support, preserving TLS verification behind organization-managed HTTPS inspection. Manual two-track validation completed locally with `faster-whisper-tiny`: German, CPU/int8, four valid monotonic segments spanning 0.031–96.725 seconds.
