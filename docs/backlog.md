...
- **AMA-116: AI hardware acceleration** — Detect NVIDIA CUDA capability, driver/runtime compatibility, GPU name and available VRAM; choose safe compute types (`float16`, `int8_float16`, `int8`) and batch sizes automatically with an explicit CPU fallback. Expose `Automatic`, `Prefer GPU` and `CPU only` settings plus active hardware and fallback reasons in the Status Center. Evaluate DirectML/AMD/Intel separately because WhisperX/CTranslate2 support and performance differ from CUDA.
