...
6. **AMA-106: Session workspace** — **Sprint 1.6.1 and 1.6.2 complete.** Each recording creates a unique `session_*` directory containing all three media artifacts and an atomically replaced, versioned `manifest.json`. A shared monotonic clock records actual provider start signals. After finalization, native WAV and MP4 parsers write each media duration, expected stream end and session-end difference into schema 2. The analyzer classifies the spread between stream ends as `aligned` at ≤100 ms or `warning` above it, and the UI displays the result. Real schema-2 validation produced a 49.6 ms end spread, independently recalculated exactly, with all artifacts present. Resume/recovery and free-disk validation remain later slices.
